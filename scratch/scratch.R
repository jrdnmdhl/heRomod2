...
state_names <- paste0('state', 1:20)
trans <- expand.grid(from = state_names, to = state_names, stringsAsFactors = F)
trans$formula <- 'ifelse(state_time <=9000 , exp(-state_time*0.05), 0.3)'

ns <- heRomod2:::create_namespace(250, 250, 1, new.env())

testit <- eval_lf_matrix(trans, ns)
microbenchmark::microbenchmark(
  a = {
    heRomod2:::eval_trans_markov_lf(trans, ns)
  },
  times = 5
)
...
state_names <- paste0('state', 1:20)
vals <- tibble(
  name = paste0(state_names, '_lys'),
  state = state_names,
  formula = 'state_time'
)

ns <- heRomod2:::create_namespace(3, 3, 1, new.env())

vals <- evaluate_values(vals, ns)
